...
const ListarEditarDetalleFacturas = ({ idInforme, onClose, onUpdate }) => {
  ...
  const fetchFacturas = useCallback(async () => {
    ...
    try {
      ...
      if (!id_usuario) {
        handleError("ID de usuario no encontrado");
        return;
      }

      const response = await fetch(
        `${API_URL}/Informes/listar-detalle-facturas/${id_usuario}/${idInforme}/`,
        {
          headers: {
            Authorization: `${token}`,
          },
  ...
  const calculateTotal = (facturas) => {
    const total = facturas.reduce(
      (sum, factura) => sum + (parseFloat(factura.valor) || 0),
      0
    );
    setTotal(total);
  };
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const response = await fetch(
        `${API_URL}/Informes/editar-justificacion/${idInforme}/`,
        {
          method: "POST",
          headers: {
  ...
  return (
    <div className="p-4">
      <h2 className="text-xl font-medium mb-4">
        Editar Justificación de Gastos
      </h2>
